[classroom_messenger_firebase.html](https://github.com/user-attachments/files/28343932/classroom_messenger_firebase.html)
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
...
.ei{font-size:36px}
.connecting{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#bbb;font-size:13px;gap:10px}
.spinner{width:28px;height:28px;border:3px solid #e0dcd8;border-top-color:var(--t);border-radius:50%;animation:spin .7s linear infinite}
...
.live-dot{width:7px;height:7px;border-radius:50%;background:#a0bb7f;display:inline-block;margin-right:5px;animation:pulse 2s infinite}
...
<div class="wrap">
  <div class="app">
    <div class="sidebar">
      <div class="sh">
        <div class="sh-title"><span class="live-dot"></span>Classroom Messenger</div>
    ...
    <div class="main">
      ...
      <div id="tv">
        ...
        <div class="msgs" id="msgs"><div class="connecting"><div class="spinner"></div>Conectando…</div></div>
        <div class="ca">
          <div class="cc">
      ...
      <div id="dv">
        ...
        <div class="board" id="board"><div class="connecting"><div class="spinner"></div>Conectando…</div></div>
      </div>
    </div>
  </div>
</div>
...
const firebaseConfig = {
  apiKey: "AIzaSyCk0xoHVl8Bt2Ouus_GudUJhPXQ2ieN7bs",
  authDomain: "classroom-petitions.firebaseapp.com",
  projectId: "classroom-petitions",
  storageBucket: "classroom-petitions.firebasestorage.app",
  messagingSenderId: "1071936565142",
  appId: "1:1071936565142:web:f019e980d59d639a9677b7"
};
...
let aid='K1', selCat='material', view='teacher', filt='all', rid=null;
let allMsgs=[];

// Nombres guardados localmente por aula
function gn(id){return localStorage.getItem('cm_name_'+id)||'';}
function sn(id,n){localStorage.setItem('cm_name_'+id,n.trim());}
...
function rSidebar(){
  document.getElementById('aulaList').innerHTML=aulas.map(a=>{
    const p=allMsgs.filter(m=>m.aid===a.id&&!m.done).length;
...
window.selAula=function(id){aid=id;rSidebar();rHead();rMsgs();if(view==='director')rBoard();}
...
window.nkey=function(e){if(e.key==='Enter'){e.preventDefault();window.saveName();}if(e.key==='Escape'){document.getElementById('ni').value=gn(aid);window.saveName();}}

// ── MENSAJES TEACHER ──
function rMsgs(){
  const el=document.getElementById('msgs');
  const ms=allMsgs.filter(m=>m.aid===aid);
...
// ── CATEGORÍA ──
window.sc=function(cat){selCat=cat;['material','urgente','info','ayuda'].forEach(c=>document.getElementById('cat-'+c).classList.toggle('selected',c===cat));}
window.hkey=function(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();window.send();}}
...
window.send=async function(){
  ...
  try {
    await addDoc(collection(db,'messages'),{
      aid, who:gn(aid), cat:selCat, txt, t, done:false, createdAt:serverTimestamp()
    });
    toast('✅ Aviso enviado');
  } catch(e){ toast('❌ Error al enviar'); console.error(e); }
}
...
// ── FILTRO MURO ──
window.fb=function(cat,btn){filt=cat;document.querySelectorAll('.fp').forEach(p=>p.classList.remove('active'));btn.classList.add('active');rBoard();}

// ── MURO DIRECTORA ──
function rBoard(){
  const el=document.getElementById('board');
  let ms=filt==='all'?allMsgs:allMsgs.filter(m=>m.cat===filt);
  ms=[...ms].reverse();
  ...
  el.innerHTML=ms.map(m=>{
    ...
    return`<div class="bm ${m.done?'done':''}">
      ...
      <div class="bb">
        ...
        <div class="bact">
          ${!m.done?`<button class="abtn aok" onclick="mkDone('${m.fbId}')">✅ Gestionado</button>`:''}
          <button class="abtn arep" onclick="openRep('${m.fbId}')">💬 Responder</button>
        </div>
      </div>
    </div>`;
  }).join('');
...
// ── MARCAR GESTIONADO EN FIREBASE ──
window.mkDone=async function(fbId){
  try{
    await updateDoc(doc(db,'messages',fbId),{done:true});
    toast('✅ Marcado como gestionado');
  }catch(e){toast('❌ Error');console.error(e);}
}

// ── MODAL RESPUESTA ──
window.openRep=function(fbId){
  rid=fbId;
  const m=allMsgs.find(x=>x.fbId===fbId);
...
window.closeM=function(){document.getElementById('modal').classList.remove('open');rid=null;}
window.cr=function(){
  const txt=document.getElementById('mta').value.trim();if(!txt)return;
  const m=allMsgs.find(x=>x.fbId===rid);
  window.closeM();toast('💬 Respuesta enviada a '+lbl(m.aid));
}
document.getElementById('modal').addEventListener('click',e=>{if(e.target===document.getElementById('modal'))window.closeM();});

// ── ESCUCHAR FIREBASE EN TIEMPO REAL ──
const q = query(collection(db,'messages'), orderBy('createdAt','asc'));
onSnapshot(q, (snapshot) => {
  allMsgs = snapshot.docs.map(d=>({fbId:d.id,...d.data()}));
  rSidebar();
  rMsgs();
  if(view==='director') rBoard();
});
